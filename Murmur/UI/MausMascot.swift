import SwiftUI

/// The OpenMausBot mascot, drawn from the same silhouette and face data the
/// app itself renders (CursorAvatar), so a bot in the dock looks like that
/// bot looks in OpenMausBot: the cursor-blob body in the bot's color
/// gradient, with white eyes. Static — the app's animation engine stays in
/// the app.
struct MausMascotView: View {
    var colorName: String?

    var body: some View {
        let base = Self.color(named: colorName)
        ZStack {
            LinearGradient(
                colors: [Self.mix(base, .white, 0.55), Color(base), Self.mix(base, .black, 0.42)],
                startPoint: .top, endPoint: .bottom
            )
            .mask(MascotShape(art: MascotArt.body))
            MascotShape(art: MascotArt.eyes).fill(.white)
        }
        .aspectRatio(1, contentMode: .fit)
        .accessibilityHidden(true)
    }

    /// The app's exact bot palette (MAUS_COLORS in the OpenMausBot source).
    static func color(named name: String?) -> NSColor {
        switch name {
        case "blue": NSColor(hex: 0x377FE6)
        case "red": NSColor(hex: 0xD94B52)
        case "orange": NSColor(hex: 0xE78531)
        case "purple": NSColor(hex: 0x8057C8)
        case "cyan": NSColor(hex: 0x0EA5C6)
        case "pink": NSColor(hex: 0xD84F8B)
        case "yellow": NSColor(hex: 0xD8A729)
        case "teal": NSColor(hex: 0x01A492)
        case "coral": NSColor(hex: 0xE5634E)
        default: NSColor(hex: 0x009957)  // green, the app's default
        }
    }

    private static func mix(_ color: NSColor, _ toward: NSColor, _ amount: CGFloat) -> Color {
        let a = color.usingColorSpace(.deviceRGB)!
        let b = toward.usingColorSpace(.deviceRGB)!
        return Color(
            red: a.redComponent + (b.redComponent - a.redComponent) * amount,
            green: a.greenComponent + (b.greenComponent - a.greenComponent) * amount,
            blue: a.blueComponent + (b.blueComponent - a.blueComponent) * amount
        )
    }
}

extension NSColor {
    convenience init(hex: Int) {
        self.init(
            red: CGFloat((hex >> 16) & 0xFF) / 255,
            green: CGFloat((hex >> 8) & 0xFF) / 255,
            blue: CGFloat(hex & 0xFF) / 255,
            alpha: 1
        )
    }
}

/// Scales a pre-built CGPath in the mascot's 258.541-unit view box into rect.
private struct MascotShape: Shape {
    let art: Path

    func path(in rect: CGRect) -> Path {
        let scale = min(rect.width, rect.height) / MascotArt.viewBox
        let transform = CGAffineTransform(translationX: rect.minX, y: rect.minY)
            .scaledBy(x: scale, y: scale)
        return art.applying(transform)
    }
}

/// The mascot's geometry, decoded once from the SVG data the app ships.
private enum MascotArt {
    /// The face box plus its 15-unit margins (CursorAvatar's VIEW_BOX).
    static let viewBox: CGFloat = 258.541

    static let body: Path = {
        // Artwork transform translate(210,80), then the silhouette fit
        // translate(-56.5564 -37.6751) scale(0.593899), then +15 into the
        // view box's positive quadrant.
        var transform = CGAffineTransform(translationX: 15 - 56.5564, y: 15 - 37.6751)
            .scaledBy(x: 0.593899, y: 0.593899)
            .translatedBy(x: 210, y: 80)
        return Path(parseSVGPath(bodyData).copy(using: &transform)!)
    }()

    static let eyes: Path = {
        // Face anchor: translate(93,101) scale(0.74) translate(-120,-122.5),
        // then +15 into the view box.
        var transform = CGAffineTransform(translationX: 15 + 93, y: 15 + 101)
            .scaledBy(x: 0.74, y: 0.74)
            .translatedBy(x: -120, y: -122.5)
        let path = CGMutablePath()
        for ring in eyeData.split(separator: "|") {
            let points = ring.split(separator: " ").compactMap { pair -> CGPoint? in
                let xy = pair.split(separator: ",")
                guard xy.count == 2, let x = Double(xy[0]), let y = Double(xy[1]) else { return nil }
                return CGPoint(x: x, y: y)
            }
            guard points.count > 2 else { continue }
            path.addLines(between: points)
            path.closeSubpath()
        }
        return Path(path.copy(using: &transform)!)
    }()

    /// Minimal absolute-command SVG path parser: the mascot uses only M/C/Z.
    private static func parseSVGPath(_ data: String) -> CGPath {
        let path = CGMutablePath()
        var numbers: [CGFloat] = []
        var command: Character = "M"
        var token = ""

        func flushToken() {
            if let value = Double(token) { numbers.append(CGFloat(value)) }
            token = ""
        }
        func apply() {
            switch command {
            case "M":
                while numbers.count >= 2 {
                    path.move(to: CGPoint(x: numbers[0], y: numbers[1]))
                    numbers.removeFirst(2)
                }
            case "L":
                while numbers.count >= 2 {
                    path.addLine(to: CGPoint(x: numbers[0], y: numbers[1]))
                    numbers.removeFirst(2)
                }
            case "C":
                while numbers.count >= 6 {
                    path.addCurve(
                        to: CGPoint(x: numbers[4], y: numbers[5]),
                        control1: CGPoint(x: numbers[0], y: numbers[1]),
                        control2: CGPoint(x: numbers[2], y: numbers[3])
                    )
                    numbers.removeFirst(6)
                }
            case "Z":
                path.closeSubpath()
            default:
                break
            }
            numbers = []
        }

        for character in data {
            if character.isLetter {
                flushToken()
                apply()
                command = character
            } else if character == " " || character == "," {
                flushToken()
            } else {
                token.append(character)
            }
        }
        flushToken()
        apply()
        return path
    }

    private static let bodyData = "M0 0 C1.12815992 0.94880479 2.25705591 1.89673511 3.38671875 2.84375 C5.57657936 4.68528228 7.75793952 6.53624249 9.93359375 8.39453125 C13.5602214 11.48647103 17.25022962 14.49819427 20.9453125 17.5078125 C25.41301487 21.15281776 29.86103386 24.8215994 34.31054688 28.48876953 C38.00933931 31.5370903 41.70951059 34.58367973 45.4140625 37.625 C52.50037463 43.44570076 59.55669812 49.29508834 66.54003906 55.23901367 C70.43289872 58.54377434 74.40406577 61.73568847 78.40625 64.90625 C82.05401433 67.85083084 85.6145398 70.89451533 89.18359375 73.93359375 C92.41424312 76.67774533 95.67698054 79.36747809 99 82 C103.47931906 85.54855146 107.83340036 89.22936876 112.18359375 92.93359375 C115.41424312 95.67774533 118.67698054 98.36747809 122 101 C125.9014198 104.09073517 129.71091352 107.27378506 133.5 110.5 C137.99002543 114.32092614 142.53350963 118.0537239 147.15234375 121.71875 C156.74255328 129.40144186 166.1812645 137.27326897 175.53833008 145.23754883 C179.4317456 148.54281661 183.40347641 151.73522157 187.40625 154.90625 C191.05401433 157.85083084 194.6145398 160.89451533 198.18359375 163.93359375 C201.41424312 166.67774533 204.67698054 169.36747809 208 172 C236.43637507 194.63776677 236.43637507 194.63776677 238.27050781 209.13867188 C239.19944445 221.27193361 237.57124038 231.13444436 230 241 C223.66050278 247.82715086 215.75482398 254.47140646 206.04764748 255.13307858 C205.3615811 255.13693349 204.67551472 255.1407884 203.96865845 255.14476013 C203.17563324 255.15165863 202.38260803 255.15855713 201.56555176 255.16566467 C200.27360901 255.16958473 200.27360901 255.16958473 198.95556641 255.17358398 C198.04112762 255.180271 197.12668884 255.18695801 196.18453979 255.19384766 C194.19843498 255.20789156 192.21231409 255.21978771 190.22618484 255.22979546 C187.07149762 255.24625057 183.91692738 255.26949556 180.76229858 255.29469299 C171.79227585 255.36530712 162.82220292 255.42526708 153.85205078 255.47680664 C148.36195434 255.5088283 142.87198905 255.55017011 137.38199997 255.59700203 C135.30028042 255.61289593 133.21853066 255.62527474 131.13676834 255.63390923 C104.46972494 255.74602279 80.75351522 259.19455182 60.52978516 278.41845703 C55.75259196 283.35727885 51.81217213 289.04473423 47.77441406 294.5859375 C44.62107661 298.87600364 41.36381878 303.08685058 38.125 307.3125 C32.82026548 314.26649347 27.55386673 321.24815866 22.3125 328.25 C21.07690581 329.89589556 19.84122979 331.5417297 18.60546875 333.1875 C16.22002164 336.36552908 13.84996009 339.55428087 11.48828125 342.75 C3.0450311 354.10095576 -5.25712203 365.22607871 -20 368 C-33.42903027 368.85957067 -44.2929604 367.90032788 -55 358.9140625 C-63.51513963 350.76480778 -67.79688328 340.99527428 -68.37686157 329.29350281 C-68.43541887 328.1487851 -68.49397617 327.00406738 -68.55430794 325.82466125 C-68.61453573 324.57243271 -68.67476353 323.32020416 -68.73681641 322.0300293 C-68.80423726 320.68369989 -68.87198477 319.33738681 -68.94003105 317.99108887 C-69.12569162 314.29881586 -69.30666938 310.60632592 -69.48688698 306.91378379 C-69.68142908 302.94481208 -69.88036562 298.97606035 -70.07873535 295.00727844 C-70.55465828 285.46711948 -71.02377004 275.92663022 -71.49235249 266.3861084 C-71.71278092 261.90166682 -71.93398876 257.41726373 -72.1552124 252.93286133 C-72.22122149 251.59460763 -72.22122149 251.59460763 -72.28856409 250.2293185 C-72.37783973 248.41936974 -72.46711776 246.6094211 -72.55639815 244.79947257 C-72.7821203 240.22326204 -73.00777832 235.64704836 -73.23336792 231.0708313 C-73.2783997 230.15734865 -73.32343148 229.24386601 -73.36982787 228.30270207 C-73.64581049 222.70253299 -73.92113412 217.10233189 -74.19602597 211.50210917 C-75.35713101 187.85146938 -76.54638525 164.20245932 -77.76320994 140.55462319 C-78.3174362 129.77404232 -78.86176258 118.9929594 -79.40472984 108.2118063 C-79.83986282 99.58021501 -80.28322749 90.94912395 -80.73695588 82.31848997 C-81.04621068 76.42094211 -81.34513355 70.52292566 -81.63612723 64.62444884 C-81.8031421 61.24551841 -81.97612174 57.86718776 -82.15861511 54.48903847 C-84.29931862 14.73242483 -84.29931862 14.73242483 -72.03125 -1.625 C-50.89854752 -24.96559677 -21.34867451 -18.24899383 0 0 Z"

    private static let eyeData = "103.39,78.25 106.56,78.38 109.59,79.28 112.29,80.94 114.43,83.28 115.86,86.1 116.52,89.2 116.32,92.37 115.62,95.46 114.86,98.55 114.1,101.63 113.36,104.72 112.63,107.81 111.91,110.91 111.2,114.01 110.51,117.1 109.82,120.21 109.16,123.31 108.52,126.42 107.84,129.53 106.8,132.52 105.04,135.16 102.66,137.25 99.81,138.63 96.7,139.2 93.54,138.96 90.53,137.95 87.87,136.24 85.71,133.92 84.21,131.13 83.48,128.05 83.5,124.88 84,121.74 84.7,118.64 85.4,115.54 86.08,112.44 86.77,109.34 87.48,106.24 88.19,103.15 88.92,100.05 89.66,96.96 90.41,93.88 91.18,90.8 91.95,87.71 93.1,84.76 94.95,82.19 97.4,80.19 100.28,78.86|161.99,91.57 165.07,92 167.9,93.28 170.32,95.23 172.22,97.69 173.49,100.53 174.05,103.59 173.93,106.7 173.45,109.78 172.9,112.85 172.33,115.91 171.71,118.97 171.06,122.02 170.38,125.06 169.66,128.09 168.91,131.12 168.13,134.14 167.33,137.15 166.5,140.16 165.58,143.14 164.33,145.98 162.5,148.5 160.15,150.53 157.38,151.95 154.34,152.61 151.24,152.43 148.32,151.38 145.83,149.52 143.99,147.01 142.95,144.08 142.73,140.98 143.25,137.91 144.09,134.91 144.94,131.91 145.76,128.9 146.54,125.89 147.29,122.86 148.01,119.83 148.69,116.79 149.34,113.74 149.95,110.68 150.54,107.62 151.09,104.55 151.6,101.47 152.44,98.48 154,95.79 156.21,93.6 158.94,92.12"
}
